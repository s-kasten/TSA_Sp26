...
```{r}
ts_inflow_data <- ts(inflow_data[,2:4], start=c(1931,1),frequency=12) #we dont want the first column converted because its a date. speficying start for the monthly data, frequency is 12 because its monthly
ts_inflow_data
...
autoplot(ts_inflow_data) +
  xlab("Time") +
  ylab("Inflow") +
  labs(color="Reservoir") #plots all the HPs unlike what would show up in ggplot. use autplot for time series plots
#if you only want one time series
autoplot(ts_inflow_data[,1], series="HP1") +
  xlab("Time") +
  ylab("Inflow") +
  labs(color="Reservoir")

#if only want 2, need to use autolayering
autoplot(ts_inflow_data[,1], series="HP1") +
...
HP1_acf= acf(ts_inflow_data[,1], lag.max=40,plot=TRUE) #also a part of this function to do partials
#the blue dashed lines are the confidence intervals, any lags between them can be considered 0 / not significant
#also only goes to lag 5 but somehow that acually means 40... idk use the captial Acf function below instead.
...
HP3_acf= Acf(ts_inflow_data[,3],lag.max=40,plot=TRUE) 
#this Acf function omits lag 0 (which is always 1), luana likes this better because it doesnt have that and is less confusing. but basically the same otherwise

#third series have less of a negative lag. interesting.

HP3_pacf= Pacf(ts_inflow_data[,3],lag.max=40,plot=TRUE)

#lags fade away a lot quicker into the CI when you calculate the PACF. see lecture notes.

HP3_acf$acf
HP3_pacf$acf #these are called acf values for pacf but they ARE PACF values. not acf values, for each lag
```
