...
can always look for NAs with summary function
```{r}
summary(wind_data)
```
to make sure no lines are missing can take 
```{r}
tail(ts_wind_data)

#it ends in december which means no data is missing from the time series (would have ended in July or something)
#or just run the whole ts_wind_data and check. you are comparing to when the original data ended
```
