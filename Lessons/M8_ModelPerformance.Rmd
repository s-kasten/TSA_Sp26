...
**object** object of class forecast, or numerical values containing forecasts. **x** numerical vector containing observed values (optional). (the arithmatic and naive outputs are the correct class but ARIMA is not (Since it doesnt forecast) and we will have to follow some steps to make it).
...
```{r}
observed <- electricity_price_processed[(nobs-n_for+1):nobs, "All.sectors"] #last 12 values for the observations that were left out of the time series originally.
#Model 1: Arithmetic mean
...
SARIMA_for <- forecast(SARIMA_autofit,h=12) #created a class forecast for the SARIMA fit - on the model not the time series, and specify the forecasting horizon
...
ts_electricity_price_full <- ts(
  electricity_price_processed[1:nobs,"All_sectors"],
  start=c(year(electricity_price_processed$Month[1]),month(electricity_price_processed$Month[1])),
  frequency=12) 
