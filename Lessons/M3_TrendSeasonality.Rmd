...
```{r acf}
#Acf and Pacf for HP1, for each value of i ACF and PACF is being plotted in the code below
for(i in 1:nhydro){
...
#Create vector t
t<- c(1:nobs)
#choose one HP to work with
iHP <- 3

#Fit a linear trend to TS of iHP
linear_trend_model <- lm(inflow_data[,iHP+1]~t)# plus one bc of date column in the data frame
...
```
not a significant regression. no overall increasing or decreasing trend when iHP was 15
it was sifniciant with HP 4. (by p value)


```{r}
#remove the trend from series
detrend_inflow_data <- inflow_data[,iHP+1] - (beta0 + beta1*t) #this is the equation of the red line
...
for(i in 1:nobs)(
 season_inflow_data[i] <- beta_intercept+beta_coeff%*%dummies[i,])
...
#plots
autoplot(ts_inflow_data[,iHP])+
  autolayer(ts_deseason_inflow_data, series = "detrend and deseries inflow")
